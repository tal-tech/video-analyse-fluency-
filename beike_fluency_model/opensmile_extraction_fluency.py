...
def opensmiler(infile, outfold, config='emobase2010',toolfold=os.path.join(base_path, 'opensmile-2.3.0/'),extension='.txt', max_retry=10):
    ...
    while not success and retry < max_retry:
        if retry > 0:
            print("{} retry {}".format(infile, retry))

        # if subprocess.call(cmd, shell=True) !=0:
        #     print('ERROR: opensmile extract {} features fail'.format(infile))
...
def batch_extract(input_wavs, output_path):
    ...
    results = {}
    # print('提取opensmile特征的音频数量: {}'.format(len(input_wavs)))
    # emobase2010
    ...
    for clip in input_wavs:
        ...
        if not os.path.exists(txt_file):
            # print("WARNING: 提取{}的opensmile特征失败，使用全0特征。".format(clip))
            ...
            results[clip] = np.array([features])
